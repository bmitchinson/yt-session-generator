...
async def run(
    ...
    potoken_extractor = PotokenExtractor(
        loop, update_interval=update_interval, browser_path=browser_path
    )
    ...
    potoken_server = PotokenServer(
        potoken_extractor, port=port, bind_address=bind_address
    )
...
Retrieve potoken using Chromium runned by nodriver, serve it on a json endpoint
    ...
    parser.add_argument(
        ...
        help="Logging level for output (default: INFO for server, WARNING for oneshot). Alternatively, set LOG_LEVEL env var.",
    )
...
def main() -> None:
    ...
    level_name = (
        ...
        or ("WARNING" if args.oneshot else "INFO")
    )
